...
impl Event {
    pub fn read(mut reader: impl Read) -> anyhow::Result<Self> {
        ...
        match kind {
            0 => {
                let event = DamageEvent::read(&mut reader)?;
                Ok(Event::Damage(event))
            }
            1 => {
                let event = SplashEvent::read(&mut reader)?;
                Ok(Event::Splash(event))
            }
            2 => {
                let event = SoundEvent::read(&mut reader)?;
...
impl DamageEvent {
    pub fn read(mut reader: impl Read) -> anyhow::Result<Self> {
        let attack_properties = AttackProperty::read(&mut reader)?;
        let elements = Element::read(&mut reader)?;
...
impl EventConditions {
    pub fn read(mut reader: impl Read) -> anyhow::Result<Self> {
        let kind = EventConditionKind::read(&mut reader)?;
